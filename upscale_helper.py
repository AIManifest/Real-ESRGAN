...
def upscale_video(args):
    args.outscale = int(args.outscale)
    input_path = args.input.rstrip('/').rstrip('\\')
    os.makedirs(args.output, exist_ok=True)

    if mimetypes.guess_type(input_path)[0] is not None and mimetypes.guess_type(input_path)[0].startswith('video'):
        is_video = True
    else:
        is_video = False
    ...
    run(args)
    print("..Video Upscaling Has Completed..File Saved to: ", f"{args.output}/{args.video_name}_{args.suffix}.mp4")
    if args.extract_frame_first:
        tmp_frames_folder = osp.join(args.output, f'{args.video_name}_inp_tmp_frames')
        shutil.rmtree(tmp_frames_folder)

def auto_upscale_image(args):
    ...
    with tqdm(total=len(paths),position=0,leave=True) as pbar:
      for idx, path in tqdm(enumerate(paths), position=0, leave=True):
        ...
        try:
            ...
            print("Saved Upscaled Image to: ", save_path)
